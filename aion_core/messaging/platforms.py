...
class PlatformAdapter(ABC):
    ...
    async def send_text(self, session_id: str, text: str) -> bool:
        ...
        if not self._connected:
            logger.warning(
                "[%s] send_text while disconnected", self.get_platform_name()
            )
            return False
        msg_id = _generate_message_id()
        self._message_buffer.append(
            _StoredMessage(
                session_id=session_id, msg_type="text", content=text, message_id=msg_id
            )
        )
        logger.info(
            "[%s] send_text → session=%s  text=%s",
    ...
    async def send_image(
        self, session_id: str, image_url: str, caption: str = ""
    ) -> bool:
        """Send an image with an optional caption."""
        if not self._connected:
            logger.warning(
                "[%s] send_image while disconnected", self.get_platform_name()
            )
            return False
        msg_id = _generate_message_id()
        self._message_buffer.append(
        ...
        logger.info(
            "[%s] send_image → session=%s  url=%s  caption=%s",
            self.get_platform_name(),
            session_id,
            _truncate(image_url, 80),
            _truncate(caption),
        )
        return True

    async def send_file(
        self, session_id: str, file_path: str, filename: str = ""
    ) -> bool:
        """Send a file attachment."""
        if not self._connected:
            logger.warning(
                "[%s] send_file while disconnected", self.get_platform_name()
            )
            return False
        msg_id = _generate_message_id()
        ...
        logger.info(
            "[%s] send_file → session=%s  path=%s  filename=%s",
            self.get_platform_name(),
            session_id,
            _truncate(file_path, 80),
            name,
        )
        return True
    ...
    async def send_audio(self, session_id: str, audio_url: str) -> bool:
        ...
        if not self._connected:
            logger.warning(
                "[%s] send_audio while disconnected", self.get_platform_name()
            )
            return False
        msg_id = _generate_message_id()
        self._message_buffer.append(
        ...
        logger.info(
            "[%s] send_audio → session=%s  url=%s",
            self.get_platform_name(),
            session_id,
            _truncate(audio_url, 80),
        )
        return True
    ...
    async def send_video(self, session_id: str, video_url: str) -> bool:
        ...
        if not self._connected:
            logger.warning(
                "[%s] send_video while disconnected", self.get_platform_name()
            )
            return False
        msg_id = _generate_message_id()
        self._message_buffer.append(
        ...
        logger.info(
            "[%s] send_video → session=%s  url=%s",
            self.get_platform_name(),
            session_id,
            _truncate(video_url, 80),
        )
        return True
    ...
    async def delete_message(self, session_id: str, message_id: str) -> bool:
        ...
        logger.info(
            "[%s] delete_message → session=%s  msg_id=%s  removed=%d",
            self.get_platform_name(),
            session_id,
            message_id,
            removed,
    ...
    async def set_typing(self, session_id: str, typing: bool = True) -> None:
        ...
        logger.info(
            "[%s] set_typing → session=%s  typing=%s",
            self.get_platform_name(),
            session_id,
            typing,
        )
...
class TelegramAdapter(PlatformAdapter):
    ...
    async def send_sticker(self, session_id: str, sticker_file_id: str) -> bool:
        ...
        logger.info(
            "[%s] sendSticker → session=%s  file_id=%s",
            self.get_platform_name(),
            session_id,
            sticker_file_id,
        )
    ...
    async def send_reply_markup(
        self, session_id: str, text: str, reply_markup: dict[str, Any]
    ) -> bool:
        ...
        logger.info(
            "[%s] sendMessage (with markup) → session=%s  text=%s",
            self.get_platform_name(),
            session_id,
            _truncate(text),
        )
    ...
    async def _poll_loop(self) -> None:
        ...
        try:
            while self._connected:
                ...
                logger.debug(
                    "[%s] getUpdates → [] (no new updates)", self.get_platform_name()
                )
        except asyncio.CancelledError:
            pass
...
class DiscordAdapter(PlatformAdapter):
    ...
    async def send_embed(self, session_id: str, embed: dict[str, Any]) -> bool:
        ...
        logger.info(
            "[%s] createMessage (embed) → session=%s  title=%s",
            self.get_platform_name(),
            session_id,
            embed.get("title", ""),
        )
    ...
    async def add_reaction(self, session_id: str, message_id: str, emoji: str) -> bool:
        ...
        logger.info(
            "[%s] addReaction → session=%s  msg=%s  emoji=%s",
            self.get_platform_name(),
            session_id,
            message_id,
            emoji,
    ...
    async def create_thread(
        self, session_id: str, name: str, initial_message: str = ""
    ) -> str | None:
        """Create a new thread in a channel. Returns thread ID."""
        thread_id = _generate_message_id()
        logger.info(
            "[%s] createThread → session=%s  name=%s  thread_id=%s",
            self.get_platform_name(),
            session_id,
            name,
            thread_id,
    ...
    async def _ws_loop(self) -> None:
        logger.info(
            "[%s] WS heartbeat loop started (interval=41.25s)", self.get_platform_name()
        )
        try:
            while self._connected:
                await asyncio.sleep(41.25)
                logger.debug(
                    "[%s] \u2764 heartbeat acknowledged", self.get_platform_name()
                )
        except asyncio.CancelledError:
            pass
...
class SlackAdapter(PlatformAdapter):
    ...
    async def send_block(self, session_id: str, blocks: list[dict[str, Any]]) -> bool:
        ...
        logger.info(
            "[%s] chat.postMessage (blocks) → session=%s  %d blocks",
            self.get_platform_name(),
            session_id,
            len(blocks),
        )
    ...
    async def add_reaction(self, session_id: str, message_id: str, emoji: str) -> bool:
        ...
        logger.info(
            "[%s] reactions.add → session=%s  msg=%s  emoji=%s",
            self.get_platform_name(),
            session_id,
            message_id,
            emoji,
    ...
    async def create_thread(
        self, session_id: str, parent_message_id: str, text: str = ""
    ) -> str | None:
        """Reply in thread. Returns thread timestamp."""
        ts = str(time.time())
        logger.info(
            "[%s] chat.postMessage (thread) → session=%s  parent=%s",
            self.get_platform_name(),
            session_id,
            parent_message_id,
        )
...
class WhatsAppAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to WhatsApp Business API (phone_id=%s)",
            self.get_platform_name(),
            self._phone_number_id[:8],
        )
        await asyncio.sleep(0.1)
        self._connected = True
    ...
    async def send_template(
        self, session_id: str, template_name: str, parameters: list[str]
    ) -> bool:
        ...
        logger.info(
            "[%s] sendTemplate → session=%s  template=%s  params=%s",
            self.get_platform_name(),
            session_id,
            template_name,
            parameters,
    ...
    async def send_interactive(
        self, session_id: str, body_text: str, buttons: list[dict[str, str]]
    ) -> bool:
        ...
        logger.info(
            "[%s] sendInteractive → session=%s  %d buttons",
            self.get_platform_name(),
            session_id,
            len(buttons),
        )
...
class SignalAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting via signal-cli (number=%s)",
            self.get_platform_name(),
            self._phone_number,
        )
        await asyncio.sleep(0.1)
        self._connected = True
        self._background_task = asyncio.create_task(
            self._receive_loop(), name="signal-rx"
        )
    ...
    async def add_reaction(self, session_id: str, message_id: str, emoji: str) -> bool:
        ...
        logger.info(
            "[%s] sendReaction → session=%s  msg=%s  emoji=%s",
            self.get_platform_name(),
            session_id,
            message_id,
            emoji,
...
class TeamsAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to Bot Framework (app_id=%s…)",
            self.get_platform_name(),
            self._app_id[:8],
        )
    ...
    async def send_adaptive_card(self, session_id: str, card: dict[str, Any]) -> bool:
        ...
        logger.info(
            "[%s] sendAdaptiveCard → session=%s", self.get_platform_name(), session_id
        )
        return True
    ...
    async def send_proactive(self, conversation_id: str, text: str) -> bool:
        ...
        logger.info(
            "[%s] proactive message → conversation=%s",
            self.get_platform_name(),
            conversation_id,
        )
        return True
...
class WeChatAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to WeChat API (corp_id=%s…)",
            self.get_platform_name(),
            self._corp_id[:8],
        )
        await asyncio.sleep(0.15)
        ...
        self._connected = True
        logger.info(
            "[%s] Connected (access_token=%s…)",
            self.get_platform_name(),
            self._access_token[:8],
        )
    ...
    async def upload_media(self, media_type: str, file_path: str) -> str | None:
        ...
        logger.info(
            "[%s] uploadMedia → type=%s  path=%s  media_id=%s",
            self.get_platform_name(),
            media_type,
            file_path,
            media_id,
    ...
    async def send_news(self, session_id: str, articles: list[dict[str, str]]) -> bool:
        ...
        logger.info(
            "[%s] sendNews → session=%s  %d articles",
            self.get_platform_name(),
            session_id,
            len(articles),
        )
...
class QQAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to QQ Bot API (app_id=%s, sandbox=%s)",
            self.get_platform_name(),
            self._app_id,
            self._sandbox,
        )
    ...
    async def send_to_guild(self, guild_id: str, channel_id: str, text: str) -> bool:
        ...
        logger.info(
            "[%s] guild message → guild=%s  channel=%s",
            self.get_platform_name(),
            guild_id,
            channel_id,
        )
    ...
    async def upload_chunked(
        self, session_id: str, file_path: str, chunk_size: int = 1024 * 1024
    ) -> str | None:
        """Upload a file using chunked upload. Returns file UUID."""
        file_uuid = hashlib.sha256(file_path.encode()).hexdigest()[:16]
        logger.info(
            "[%s] chunked upload → session=%s  path=%s  chunk_size=%d  uuid=%s",
            self.get_platform_name(),
            session_id,
            file_path,
            chunk_size,
...
class FeishuAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to Feishu Open API (app_id=%s…)",
            self.get_platform_name(),
            self._app_id[:8],
        )
        await asyncio.sleep(0.12)
        ...
        self._connected = True
        self._background_task = asyncio.create_task(
            self._event_loop(), name="feishu-events"
        )
        logger.info(
            "[%s] Connected (tenant_token=%s…)",
            self.get_platform_name(),
            self._tenant_token[:8],
        )
    ...
    async def create_meeting(
        self, title: str, start_time: str, duration_minutes: int
    ) -> str | None:
        ...
        logger.info(
            "[%s] createMeeting → title=%s  duration=%dm  url=%s",
            self.get_platform_name(),
            title,
            duration_minutes,
            url,
...
class WeixinWorkAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to WeCom API (corp_id=%s…)",
            self.get_platform_name(),
            self._corp_id[:8],
        )
        await asyncio.sleep(0.12)
        ...
        self._connected = True
        logger.info(
            "[%s] Connected (agent_id=%s)", self.get_platform_name(), self._agent_id
        )
...
class YuanbaoAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to Yuanbao API (app_id=%s…)",
            self.get_platform_name(),
            self._app_id[:8],
        )
        await asyncio.sleep(0.1)
        self._connected = True
        logger.info(
            "[%s] Connected (endpoint=%s)", self.get_platform_name(), self._endpoint
        )
    ...
    async def send_rich_text(
        self, session_id: str, content: str, format_type: str = "markdown"
    ) -> bool:
        ...
        logger.info(
            "[%s] sendRichText → session=%s  format=%s",
            self.get_platform_name(),
            session_id,
            format_type,
        )
...
class MatrixAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        self._connected = True
        self._background_task = asyncio.create_task(
            self._sync_loop(), name="matrix-sync"
        )
        logger.info(
            "[%s] Connected — initial /sync completed", self.get_platform_name()
        )
    ...
    async def send_encrypted(self, session_id: str, text: str) -> bool:
        ...
        self._message_buffer.append(
            _StoredMessage(
                session_id=session_id,
                msg_type="encrypted_text",
                content=text,
                message_id=msg_id,
            )
        )
        logger.info(
            "[%s] sendEncrypted → room=%s  (E2EE)", self.get_platform_name(), session_id
        )
        return True

    async def send_html(
        self, session_id: str, html_body: str, text_body: str = ""
    ) -> bool:
...
class IRCAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to %s:%d as %s (ssl=%s)",
            self.get_platform_name(),
            self._server,
            self._port,
            self._nick,
...
class MattermostAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to %s (team=%s)",
            self.get_platform_name(),
            self._url,
            self._team_name or "default",
        )
        await asyncio.sleep(0.1)
        self._connected = True
        self._background_task = asyncio.create_task(
            self._ws_loop(), name="mattermost-ws"
        )
    ...
    async def send_slash_command(self, session_id: str, command: str) -> bool:
        ...
        logger.info(
            "[%s] slashCommand → session=%s  cmd=%s",
            self.get_platform_name(),
            session_id,
            command,
        )
...
class LineAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to LINE Messaging API (token=%s…)",
            self.get_platform_name(),
            self._channel_token[:8],
        )
        await asyncio.sleep(0.1)
        self._connected = True
        logger.info("[%s] Connected", self.get_platform_name())
    ...
    async def send_flex_message(
        self, session_id: str, alt_text: str, contents: dict[str, Any]
    ) -> bool:
    ...
    async def push_message(self, session_id: str, text: str) -> bool:
        ...
        self._message_buffer.append(
            _StoredMessage(
                session_id=session_id, msg_type="push", content=text, message_id=msg_id
            )
        )
        logger.info(
            "[%s] push → session=%s  text=%s",
            self.get_platform_name(),
            session_id,
            _truncate(text),
        )
...
class GoogleChatAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to Google Chat API (project=%s)",
            self.get_platform_name(),
            self._project_id or "unknown",
        )
        await asyncio.sleep(0.2)
        self._access_token = hashlib.sha256(self._service_account.encode()).hexdigest()[
            :32
        ]
        self._connected = True
        logger.info(
            "[%s] Connected — service account authenticated", self.get_platform_name()
        )
    ...
    async def send_threaded_reply(self, thread_name: str, text: str) -> bool:
        ...
        logger.info(
            "[%s] threaded reply → thread=%s", self.get_platform_name(), thread_name
        )
        return True
...
class DingTalkAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to DingTalk API (app_key=%s…)",
            self.get_platform_name(),
            self._app_key[:8],
        )
        await asyncio.sleep(0.12)
        ...
        self._connected = True
        self._background_task = asyncio.create_task(
            self._stream_loop(), name="dingtalk-stream"
        )
    ...
    async def send_action_card(
        self, session_id: str, title: str, text: str, btn_title: str, btn_url: str
    ) -> bool:
        ...
        self._message_buffer.append(
            _StoredMessage(
                ...
                content=text,
                extra={"title": title, "btn_title": btn_title, "btn_url": btn_url},
                message_id=msg_id,
            )
        )
        logger.info(
            "[%s] sendActionCard → session=%s  title=%s",
            self.get_platform_name(),
            session_id,
            title,
        )
    ...
    async def send_markdown(self, session_id: str, title: str, text: str) -> bool:
        ...
        logger.info(
            "[%s] sendMarkdown → session=%s  title=%s",
            self.get_platform_name(),
            session_id,
            title,
        )
...
class EmailAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to SMTP %s:%d (user=%s)",
            self.get_platform_name(),
            self._smtp_host,
            self._smtp_port,
            self._smtp_user,
    ...
    async def send_email(
        ...
        logger.info(
            "[%s] sendEmail → to=%s  subject=%s  attachments=%d",
            self.get_platform_name(),
            to,
            subject,
            len(attachments or []),
...
class NtfyAdapter(PlatformAdapter):
    ...
    async def connect(self) -> None:
        ...
        logger.info(
            "[%s] Connecting to %s (topic=%s, priority=%d)",
            self.get_platform_name(),
            self._server_url,
            self._topic,
            self._priority,
    ...
    async def send_notification(
        ...
        logger.info(
            "[%s] publish → topic=%s  title=%s  priority=%d  tags=%s",
            self.get_platform_name(),
            self._topic,
            title,
            p,
    ...
    async def send_with_actions(
        self, title: str, body: str, actions: list[dict[str, str]]
    ) -> bool:
        ...
        self._message_buffer.append(
            _StoredMessage(
                session_id=self._topic,
                msg_type="notification_actions",
                content=body,
                extra={"title": title, "actions": actions},
                message_id=msg_id,
            )
        )
        logger.info(
            "[%s] publish (actions) → topic=%s  %d actions",
            self.get_platform_name(),
            self._topic,
            len(actions),
        )
...
class WebhookAdapter(PlatformAdapter):
    ...
    async def fire(self, payload: dict[str, Any], override_url: str = "") -> bool:
        ...
        self._message_buffer.append(
            _StoredMessage(
                session_id=target,
                msg_type="webhook_fire",
                content=json.dumps(payload),
                message_id=msg_id,
            )
        )
        logger.info(
            "[%s] fire → %s %s  payload=%s",
            self.get_platform_name(),
            self._method,
            target,
    ...
    async def fire_raw(
        ...
        self._message_buffer.append(
            _StoredMessage(
                session_id=self._url,
                msg_type="webhook_raw",
                content=body,
                extra={"content_type": content_type, "headers": headers},
                message_id=msg_id,
            )
        )
        logger.info(
            "[%s] fire_raw → %s %s  content_type=%s  len=%d",
            self.get_platform_name(),
            self._method,
            self._url,
            content_type,
...
class PlatformRegistry:
    ...
    def register(self, platform_type: PlatformType, config: dict[str, Any]) -> None:
        ...
        logger.info(
            "PlatformRegistry: registered %s (%s)",
            platform_type.value,
            type(adapter).__name__,
        )

    def register_instance(
        self, platform_type: PlatformType, adapter: PlatformAdapter
    ) -> None:
        """Register a pre-created adapter instance."""
        self._adapters[platform_type] = adapter
        logger.info(
            "PlatformRegistry: registered instance %s (%s)",
            platform_type.value,
            type(adapter).__name__,
        )
    ...
    async def connect_all(self) -> dict[PlatformType, bool]:
        ...
        for pt, outcome in zip(coros.keys(), outcomes):
            if isinstance(outcome, Exception):
                logger.error(
                    "PlatformRegistry: failed to connect %s — %s", pt.value, outcome
                )
                results[pt] = False
            else:
                results[pt] = True
        logger.info(
            "PlatformRegistry: connect_all → %d/%d succeeded",
            sum(results.values()),
            len(results),
        )
        return results
...
def validate_platform_config(
    ...
    if platform_type == PlatformType.DISCORD and "intents" in config:
        ...
        if not isinstance(intents, list):
            errors.append(
                f"Discord intents must be a list, got {type(intents).__name__}"
            )
