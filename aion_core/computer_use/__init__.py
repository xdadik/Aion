...
class ComputerUse:
    ...
    async def screenshot(self, output_path: Path | str | None = None) -> Path:
        ...
        if output_path is None:
            output_path = (
                ...
                / f"aion_screen_{asyncio.get_event_loop().time():.0f}.png"
            )
        ...
        raise RuntimeError(
            "No screenshot backend available. Install: pip install pillow pynput"
        )
    ...
    async def mouse_click(
        ...
        if self._backend == "pil+pynput":
            ...
            def _click() -> None:
                from pynput.mouse import (  # type: ignore[import-not-found]
                    Button,
                    Controller,
                )
    ...
    async def keyboard_press(self, key: str) -> None:
        ...
        if self._backend == "pil+pynput":
            ...
            def _press() -> None:
                from pynput.keyboard import (  # type: ignore[import-not-found]
                    Controller,
                    Key,
                )
